                    ...
                    <tbody>
                        @forelse ($usersWithUpdateStatus as $user)
                            <tr class="hover:bg-red-50">
                                <td class="border border-gray-300">{{ $user['user']->name }}</td>
                                <td class="border border-gray-300">{{ $user['user']->no_hp }}</td>
                                <td class="border border-gray-300">{{ $store->toko_code }}</td>
                                <td class="border border-gray-300">{{ $user['user']->position->position_name }}</td>
                                <td class="border border-gray-300">{{ $user['user']->position->grade->max_grade }}</td>
                                <td class="border border-gray-300">
                                    @if ($user['canUpdate'])
                                        @if (url()->current() ===
                                                route('department.area.stores.employees.position.index', [
                                                    'departmentUuid' => $user['user']->department->uuid,
                                                    'tokoCode' => $store->toko_code,
                                                    'userName' => $user['user']->name,
                                                ]))
                                            <a href="{{ route('department.area.stores.employees.position.edit', ['departmentUuid' => $user['user']->department->uuid, 'tokoCode' => $store->toko_code, 'userName' => $user['user']->name, 'userUuid' => $user['user']->uuid]) }}"
                                                class="btn btn-sm btn-info"><i
                                                    class="fa-regular fa-pen-to-square"></i>Update</a>
                                        @else
                                            <a href="{{ route('department.area.stores.employees.edit', ['userUuid' => $user['user']->uuid, 'departmentUuid' => $user['user']->department->uuid, 'tokoCode' => $store->toko_code]) }}"
                                                class="btn btn-sm btn-info"><i
                                                    class="fa-regular fa-pen-to-square"></i>Update</a>
                                        @endif
                                    @else
