        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <div class="p-6 text-gray-900">
                ...
                <a href="@if ($userDivision === 'O2222' && $userDept === 'O1900') {{ route('warehouse.dashboard') }}
                    ...
                    class="btn btn-info btn-md mb-6">
                    Kembali
                </a>
                ...
                <form action="{{ route('department.search') }}" method="GET" class="my-6 flex items-center gap-4">
                    <input type="text" name="query" placeholder="Cari department" value="{{ request('query') }}"
                        class="md:w-1/3 sm:w-1/2 border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-red-500">
                    <button type="submit" class="btn bg-blue-500 px-6 py-3 hover:bg-blue-600 transition">
                        Cari
                    </button>
                </form>
                ...
                <div class="fixed top-5 inset-x-0 md:inset-x-1/2 z-50 sm:w-full md:w-1/2 lg:w-1/3">
                    ...
                    @if (session('error'))
                        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 2000)" x-show="show"
                            x-transition:leave="transition ease-in duration-300"
                            class="alert alert-error mb-4 bg-red-500">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 stroke-current"
                                fill="none" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        @else
                                            @if ($department->department_code === 'O1200')
                                                <a
                                                    href="{{ route('department.area.stores.index', $department->uuid) }}"class="text-blue-500 font-semibold hover:underline">Lihat
                                                    Detail Area </a>
                                            @else
                                                <p class="text-gray-700">Karyawan: <span
                                                        class="font-bold">{{ $department->users_count }}</span></p>
